...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        MotionSensors calib = new MotionSensors(this);
        calib.getRotationMatrix();
        ...
        for(int i = 0; i < nearbyPicturesList.size() && !imageFound; i++) {
            ...
            if(!pictureIsViewed(currentPictureIndex)) {
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        sceneRenderer.addTable(getApplicationContext(), nearbyPicturesList.get(currentPictureIndex).getPicture(), nearbyPicturesList.get(currentPictureIndex).getScale());
                    }
                });
            }
        }
